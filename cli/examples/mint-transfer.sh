...
ms=$(multisig new $threshold $owners)
...
tx=$(multisig propose-mint-tokens $multisig $mint $ms_token_account 100 | tee /dev/tty | tail -n1)
echo tx $tx
...
tx=$(multisig propose-transfer-tokens $multisig $ms_token_account $my_token_account 50 | tee /dev/tty | tail -n1)
echo tx $tx
echo 'sleeping so network can reconcile account ownership (not sure why this is necessary but it never works immediately)'
sleep 14
multisig approve $multisig $tx
